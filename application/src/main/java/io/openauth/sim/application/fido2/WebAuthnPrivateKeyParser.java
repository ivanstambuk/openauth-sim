...
final class WebAuthnPrivateKeyParser {
    ...
    static ParsedKey parse(String value, WebAuthnSignatureAlgorithm algorithm) throws GeneralSecurityException {
    ...
    static WebAuthnSignatureAlgorithm inferAlgorithm(String value) throws GeneralSecurityException {
        ...
        if (trimmed.startsWith(PEM_BEGIN)) {
            String normalized = trimmed.replaceAll("-----BEGIN [^-]+-----", "")
                    .replaceAll("-----END [^-]+-----", "")
                    .replaceAll("\\s", "");
            byte[] der;
            try {
            ...
            try {
                PrivateKey privateKey = KeyFactory.getInstance("Ed25519").generatePrivate(new PKCS8EncodedKeySpec(der));
                if (privateKey != null) {
                    return WebAuthnSignatureAlgorithm.EDDSA;
                }
            } catch (GeneralSecurityException ignored) {
    ...
    private static ParsedKey parsePem(String pem, WebAuthnSignatureAlgorithm algorithm)
        ...
        if (pem.contains("EC PRIVATE KEY")) {
            throw new GeneralSecurityException("SEC1 EC private keys are not supported; provide PKCS#8 PRIVATE KEY");
        }
        if (pem.contains("RSA PRIVATE KEY")) {
            throw new GeneralSecurityException("PKCS#1 RSA private keys are not supported; provide PKCS#8 PRIVATE KEY");
        }
    ...
    private static ParsedKey parseEcJwk(String kty, Map<String, String> fields, WebAuthnSignatureAlgorithm algorithm)
            throws GeneralSecurityException {
        if (!"EC".equalsIgnoreCase(kty)) {
    ...
    static ParsedKey fromFixtureKey(String base64Url, WebAuthnSignatureAlgorithm algorithm, PublicKey publicKey)
            throws GeneralSecurityException {
    ...
    private static ParsedKey deriveEcFixtureKey(
        ...
        if (!(publicKey instanceof ECPublicKey ecPublicKey)) {
            throw new GeneralSecurityException("Expected EC public key for algorithm " + algorithm.label());
        }
        ...
        builder.add("\"x\":\""
                + URL_ENCODER.encodeToString(toUnsigned(ecPublicKey.getW().getAffineX(), coordinateLength(algorithm)))
                + "\"");
        builder.add("\"y\":\""
                + URL_ENCODER.encodeToString(toUnsigned(ecPublicKey.getW().getAffineY(), coordinateLength(algorithm)))
                + "\"");
    ...
    private static ParsedKey deriveRsaFixtureKey(String base64Url, PublicKey publicKey)
        ...
        String modulus = URL_ENCODER.encodeToString(toUnsigned(rsaPublicKey.getModulus(), modulusLength));
        String exponent = URL_ENCODER.encodeToString(
                toUnsigned(rsaPublicKey.getPublicExponent(), unsignedLength(rsaPublicKey.getPublicExponent())));
    ...
    private static ParsedKey deriveEdFixtureKey(String base64Url, PublicKey publicKey) throws GeneralSecurityException {
    ...
    private static byte[] extractEd25519PublicKey(PublicKey publicKey) throws GeneralSecurityException {
    ...
    private static ParsedKey encodeEcPrivateKey(ECPrivateKey privateKey, WebAuthnSignatureAlgorithm algorithm)
            throws GeneralSecurityException {
        ...
        builder.add("\"crv\":\"" + curveForAlgorithm(algorithm) + "\"");
        builder.add("\"x\":\""
                + URL_ENCODER.encodeToString(toUnsigned(publicPoint.getAffineX(), coordinateLength(algorithm)))
                + "\"");
        builder.add("\"y\":\""
                + URL_ENCODER.encodeToString(toUnsigned(publicPoint.getAffineY(), coordinateLength(algorithm)))
                + "\"");
        builder.add("\"d\":\"" + canonical + "\"");
    ...
    private static ParsedKey encodeRsaPrivateKey(RSAPrivateKey privateKey) throws GeneralSecurityException {
        ...
        StringJoiner builder = new StringJoiner(",", "{", "}");
        builder.add("\"kty\":\"RSA\"");
        builder.add("\"n\":\"" + URL_ENCODER.encodeToString(toUnsigned(privateKey.getModulus(), length)) + "\"");
        builder.add("\"e\":\""
        ...
        builder.add("\"d\":\"" + canonical + "\"");
        builder.add("\"p\":\""
                + URL_ENCODER.encodeToString(toUnsigned(crt.getPrimeP(), unsignedLength(crt.getPrimeP())))
                + "\"");
        builder.add("\"q\":\""
                + URL_ENCODER.encodeToString(toUnsigned(crt.getPrimeQ(), unsignedLength(crt.getPrimeQ())))
                + "\"");
        builder.add("\"dp\":\""
                + URL_ENCODER.encodeToString(
    ...
    private static ParsedKey encodeEdPrivateKey(PrivateKey privateKey) throws GeneralSecurityException {
    ...
    private static ECParameterSpec ensureParams(ECParameterSpec params, WebAuthnSignatureAlgorithm alg)
            throws GeneralSecurityException {
        if (params != null) {
            return params;
        }
