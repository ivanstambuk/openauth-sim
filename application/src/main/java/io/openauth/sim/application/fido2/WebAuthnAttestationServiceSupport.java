...
final class WebAuthnAttestationServiceSupport {
  ...
  static Outcome process(
    ...
    boolean anchorTrusted =
        anchorProvided
            ...
            && matchesAnchor(certificateChain, trustAnchors);
    boolean selfAttestedFallback =
        success && (!anchorProvided || certificateChain.isEmpty() || !anchorTrusted);
  ...
  private static boolean matchesAnchor(
      List<X509Certificate> certificateChain, List<X509Certificate> trustAnchors) {
  ...
  private static String determineAnchorSource(
    ...
    return switch (source) {
      ...
      case COMBINED ->
          anchorTrusted
              ? "metadata_manual"
              : (success ? "metadata_manual_unmatched" : "metadata_manual");
      case MANUAL, NONE ->
          anchorTrusted ? "provided" : (success ? "provided_unmatched" : "provided");
    };
  }
}
