...
public final class WebAuthnAssertionGenerationApplicationService {
  ...
  private GenerationResult generateInline(GenerationCommand.Inline command) {
    try {
      ...
      byte[] clientDataJson =
          createClientDataJson(command.expectedType(), challenge, command.origin());
      ...
      byte[] signature =
          signAssertion(
              command.algorithm(), keyMaterial.privateKey(), authenticatorData, clientDataJson);
      ...
      throw new IllegalArgumentException(
          "Unable to generate WebAuthn assertion: " + ex.getMessage(), ex);
    }
  }
  ...
  private GenerationResult generateStored(GenerationCommand.Stored command) {
    ...
    long signatureCounter =
        command.signatureCounterOverrideValue().orElse(descriptor.signatureCounter());
    boolean userVerificationRequired =
        command
            .userVerificationRequiredOverrideValue()
            .orElse(descriptor.userVerificationRequired());

    GenerationCommand.Inline inline =
  ...
  private static byte[] buildAuthenticatorData(
      String relyingPartyId, long counter, boolean userVerified) throws GeneralSecurityException {
  ...
  private static byte[] signAssertion(
    ...
    System.arraycopy(
        clientDataHash, 0, signedPayload, authenticatorData.length, clientDataHash.length);
  ...
  private static KeyMaterial parseEcJwk(
  ...
  private static PublicKey deriveEcPublicKey(
      PrivateKey privateKey, WebAuthnSignatureAlgorithm algorithm) throws GeneralSecurityException {
  ...
  public sealed interface GenerationCommand
      permits GenerationCommand.Inline, GenerationCommand.Stored {
    ...
    record Stored(
      ...
      public Stored {
        credentialName = Objects.requireNonNull(credentialName, "credentialName").trim();
