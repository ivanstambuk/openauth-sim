...
public final class HotpIssuanceApplicationService {
    ...
    try {
      ...
      HotpDescriptor descriptor =
          HotpDescriptor.create(
              command.credentialId(), secret, command.algorithm(), command.digits());
  ...
  private TelemetrySignal issuanceSuccessSignal(
      IssuanceCommand command, int metadataSize, boolean created) {
  ...
  private TelemetrySignal issuanceValidationFailure(
      IssuanceCommand command, String reason, int metadataSize) {
  ...
  private TelemetrySignal issuanceError(
      IssuanceCommand command, Throwable error, int metadataSize) {
    Map<String, Object> fields = issuanceFields(command, metadataSize, false);
    if (error != null) {
  ...
  private IssuanceResult typeMismatchResult(
      IssuanceCommand command, CredentialType actualType, int metadataSize) {
    Map<String, Object> fields = issuanceFields(command, metadataSize, false);
  ...
  private Map<String, Object> issuanceFields(
      IssuanceCommand command, int metadataSize, boolean created) {
  ...
  private Map<String, String> buildAttributes(
      IssuanceCommand command, Map<String, String> metadata) {
