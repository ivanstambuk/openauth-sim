...
final class WebAuthnPublicKeyFormats {
    ...
    static byte[] encodePublicKey(PublicKey publicKey, WebAuthnSignatureAlgorithm algorithm)
        ...
        throw new GeneralSecurityException("Unsupported public key algorithm: " + publicKey.getAlgorithm());
    }
    ...
    static byte[] encodeRsaPublicKey(RSAPublicKey publicKey, WebAuthnSignatureAlgorithm algorithm)
        ...
        CborWriter writer = new CborWriter();
        writer.startMap(4);
        writer.writeBytes(-2, toUnsigned(publicKey.getPublicExponent(), unsignedLength(publicKey.getPublicExponent())));
        writer.writeBytes(-1, toUnsigned(publicKey.getModulus(), unsignedLength(publicKey.getModulus())));
        writer.writeInt(1, 3); // kty: RSA
        int coseAlg = algorithm == WebAuthnSignatureAlgorithm.PS256
    ...
    static byte[] encodeOkpPublicKey(byte[] publicKey, WebAuthnSignatureAlgorithm algorithm)
            throws GeneralSecurityException {
        if (algorithm != WebAuthnSignatureAlgorithm.EDDSA) {
            throw new GeneralSecurityException("Unsupported OKP algorithm for COSE encoding: " + algorithm.label());
        }
    ...
    private static byte[] decodeEcJwk(String kty, Map<String, Object> jwk, WebAuthnSignatureAlgorithm algorithm)
            throws GeneralSecurityException {
    ...
    private static byte[] decodeRsaJwk(String kty, Map<String, Object> jwk, WebAuthnSignatureAlgorithm algorithm)
            throws GeneralSecurityException {
    ...
    private static byte[] decodeOkpJwk(String kty, Map<String, Object> jwk, WebAuthnSignatureAlgorithm algorithm)
            throws GeneralSecurityException {
