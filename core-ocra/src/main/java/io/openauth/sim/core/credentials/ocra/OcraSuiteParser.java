...
final class OcraSuiteParser {
    ...
    private static OcraCryptoFunction parseCryptoFunction(String rawCrypto) {
        ...
        if (!cursor.startsWith("HOTP")) {
            throw new IllegalArgumentException("Only HOTP-derived OCRA suites are supported: " + rawCrypto);
        }
    ...
    private static OcraDataInput parseDataInput(String rawDataInput, OcraCryptoFunction cryptoFunction) {
        ...
        for (String token : tokens) {
            ...
            if (trimmed.startsWith("P")) {
                if (pinSpecification != null) {
                    throw new IllegalArgumentException("Multiple PIN declarations not supported: " + rawDataInput);
                }
