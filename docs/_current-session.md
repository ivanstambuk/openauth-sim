...
- Last green commands: `./gradlew --no-daemon jacocoCoverageVerification` (2025-10-25), `./gradlew --no-daemon checkstyleMain checkstyleTest` (2025-10-25), `./gradlew --no-daemon spotlessApply check` (2025-10-25), `./gradlew --no-daemon :rest-api:test --tests "io.openauth.sim.rest.Fido2*Test"` (2025-10-25), `OPENAPI_SNAPSHOT_WRITE=true ./gradlew --no-daemon :rest-api:test --tests "io.openauth.sim.rest.OpenApiSnapshotTest"` (2025-10-25), `./gradlew --no-daemon :rest-api:test --tests "io.openauth.sim.rest.Fido2ReplayEndpointTest.inlineReplayWithExtensionsReturnsExtensionMetadata"` (2025-10-25), `./gradlew --no-daemon :rest-api:test --tests "io.openauth.sim.rest.ui.Fido2OperatorUiSeleniumTest.verboseTraceSurfacesExtensionMetadataForInlineReplay"` (2025-10-25).
...
- Outstanding git state: Feature 035 docs/tasks updated through T3542 (parser + Selenium wait); next focus is Feature 036 tier helper groundwork followed by T3543 documentation sync.
...
| Feature 035 – Evaluate & Replay Audit Tracing | In progress | T3542 (Shared authenticator-data parser + Selenium wait fix) | T3543 – Facade documentation & trace sample sync | Extension parser now feeds application/CLI/REST traces, REST inline replay test enforces decoded metadata, and Selenium inline replay coverage waits for the populated badge before asserting extension attributes. Documentation/OpenAPI refresh remains outstanding. |
