...
- Last green commands: `OPENAUTH_SIM_PERSISTENCE_DATABASE_PATH=build/tmp/test-credentials.db ./gradlew --no-daemon spotlessApply check` (executed 2025-11-06 after stored-mode credential inputs were converted to CSS-based hiding and the EMV console suites reran).
...
| Feature 040 – EUDIW OpenID4VP Simulator | Ready for implementation | Specification, plan, and tasks baselined with HAIP-aligned scope; analysis gate completed 2025-11-06 | T4001 – Fixture scaffolding & seed setup (red tests) | Remote OpenID4VP simulator documented (DCQL, SD-JWT VC, mdoc, Trusted Authorities, HAIP encryption) with Evaluate/Replay tabs, inline/stored credential selectors, friendly Trusted Authority labels, ETSI TL/OpenID Federation ingestion, global verbose trace integration, and Generate/Validate flows; trusted authority snapshot + synthetic key fixtures and loaders landed 2025-11-06; hybrid fixture strategy and facade integrations queued behind tests-first increments. |
