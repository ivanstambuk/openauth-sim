...
| Feature 040 – EUDIW OpenID4VP Simulator | In progress | T4005 – SD-JWT wallet implementation recovery | T4006 – mdoc wallet tests | Authorization request + wallet services restored from compiled artefacts; SD-JWT wallet implementation (Option A hashing) and fixtures are back in place with green `:core:test`/`:application:test`. Next increment resumes planned work on mdoc wallet coverage (T4006). |
