...
- Outstanding git state: Feature 035 follow-up limited to OCRA message integrity docs (T3529); tier gating now tracks under Feature 036.
...
| Feature 035 – Evaluate & Replay Audit Tracing | In progress | T3536 (WebAuthn signature base length & preview metadata) | T3529 – OCRA message integrity summary | WebAuthn traces now report signature-base lengths, full hex, and previews across application/CLI layers with targeted tests (`:application:test` verbose suites, `:cli:test` FIDO2). Remaining scope covers OCRA message integrity docs (T3529). |
