...
- Last green commands: `./gradlew --no-daemon :core:test :application:test` (2025-11-07 – T4007 mdoc wallet implementation) and the full `./gradlew --no-daemon spotlessApply check` (2025-11-07, ~7m38s covering Feature 039 UI suites + Jacoco aggregation).
- Build status: Feature 037 shared-secret polish (T3707) wrapped; Feature 038 remains complete with aligned UI previews and accessibility documentation. Feature 039 delivered stored-mode UI hardening on 2025-11-06—stored evaluations now hide master key/CDOL1/IPB/ICC/IAD field groups via `emv-stored-mode` CSS while inline flows regain pointer events; Selenium + JS suites updated and full Gradle quality gate rerun (inline Sign replay Selenium scenario temporarily disabled pending T3936 inline hydration). Feature 026 (FIDO2/WebAuthn) stored sanitisation (I45/T2650) is implemented and awaiting owner acceptance. Feature 029 PMD hardening (T2902) continues in parallel. Feature 040 now has S2 T4007 green: the mdoc wallet service consumes the new DeviceResponse fixtures, enforces Trusted Authority policies, emits claims-pointer diagnostics, and `./gradlew --no-daemon :core:test :application:test` plus the full `spotlessApply check` are back to green.
...
| Feature 040 – EUDIW OpenID4VP Simulator | In progress | T4009 – Trusted Authority evaluator integration | T4009 – Validation plumbing | Authorization + SD-JWT/mdoc wallet services now consume `TrustedAuthorityEvaluator`, emit friendly Trusted Authority verdicts in responses/traces/telemetry, and raise RFC 7807 (`invalid_scope`) problem-details via `Oid4vpValidationException` when policies fail. REST received `Oid4vpProblemDetailsAdvice` + response DTO, and CLI gained `Oid4vpProblemDetailsFormatter`, so facades inherit the same payloads; `./gradlew --no-daemon :application:test`, `:cli:test`, and `:rest-api:test` all pass. Next step is incorporating the evaluator/problem-details into the forthcoming validation service and documenting the telemetry redaction guidance before moving to encryption tasks.
