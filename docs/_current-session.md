...
| Feature 040 – EUDIW OpenID4VP Simulator | In progress | T4007 – mdoc wallet implementation | T4008 – Trusted Authorities tests | Authorization + SD-JWT wallet refactors now ship inline-only support, disclosure hash recomputation, Trusted Authority telemetry, and deterministic trace/telemetry payloads (greens: `:application:test --tests "...OpenId4VpWalletSimulationServiceTest"` + `spotlessApply check`). Mdoc wallet service now consumes DeviceResponse fixtures, publishes claims-pointer diagnostics, and keeps HAIP encryption hooks engaged; next increment moves to Trusted Authority evaluator coverage (T4008/T4009).
