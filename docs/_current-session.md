...
- Outstanding git state: Synthetic PS256 fixture + seeding/test fallbacks, CLI/REST/UI doc updates, stored replay dropdown wait fix (spec/plan/tasks + Selenium), stored replay read-only styling parity (spec/plan/tasks + `console.css`), and roadmap/knowledge map edits staged locally; Feature 035 docs/tasks updated through T3545 (trace restructure complete); Feature 036 tier helper groundwork remains the next focus.
...
| Feature 035 – Evaluate & Replay Audit Tracing | In progress | I24 – WebAuthn stored credential ordering alignment | Follow-up: Feature 036 tier helpers | Operator console stored credential dropdowns now follow the canonical algorithm order (ES256→EdDSA) across REST responses and UI rendering; Selenium coverage updated and full `./gradlew --no-daemon spotlessApply check` completed on 2025-10-26. Tier-control work continues under Feature 036. |
