...
- Last green commands: `./gradlew --no-daemon :application:test --tests "io.openauth.sim.application.emv.cap.*"`, `./gradlew --no-daemon :cli:test --tests "io.openauth.sim.cli.EmvCli*"`, `OPENAPI_SNAPSHOT_WRITE=true ./gradlew --no-daemon :rest-api:test --tests "io.openauth.sim.rest.OpenApiSnapshotTest"`, `./gradlew --no-daemon :rest-api:test --tests "io.openauth.sim.rest.emv.cap.*"`, `./gradlew --no-daemon :rest-api:test --tests "io.openauth.sim.rest.emv.cap.EmvCapReplayEndpointTest"`, `./gradlew --no-daemon :rest-api:test --tests "io.openauth.sim.rest.emv.cap.EmvCapEvaluationEndpointTest"`, `./gradlew --no-daemon :rest-api:test --tests "io.openauth.sim.rest.ui.EmvCapOperatorUiSeleniumTest"`, `./gradlew --no-daemon :ui:test`, and `./gradlew --no-daemon spotlessApply check` (2025-11-02–11-03 stored evaluate rollout).
- Build status: Feature 037 shared-secret polish (T3707) wrapped; Feature 038 remains complete with aligned UI previews and accessibility documentation. Feature 039 reopened for I21/T3922 (evaluate mode toggle), I22/T3923 (preview window controls), and I23/T3924 (verbose trace diagnostics) while the `spotlessApply check` baseline stays green (Jacoco branches 2161/3087 ≈0.7000). Feature 029 PMD hardening (T2902) stays in flight as the other active increment.
- Handoff note (2025-11-03): Feature 037 remains ready to archive; Feature 038 stays closed after accessibility review, documentation sync, and full quality gate. Feature 039 now tracks I21/T3922 (evaluate toggle), I22/T3923 (preview windows), and I23/T3924 (verbose diagnostics) in the spec/plan/tasks/roadmap backlog. Feature 029 PMD hardening (T2902) remains the parallel focus.
...
|------------|--------|----------------|----------------|-------|
| Feature 039 – EMV/CAP Simulation Services | In progress | T3921 – UI transaction override removal | T3922 – Operator UI evaluate mode toggle alignment → T3924 – Verbose trace diagnostic parity | Reopened to ship evaluate mode toggle, preview window controls (UI/REST/CLI), and verbose trace diagnostic parity before seeking acceptance; baseline build remains green. |
