...
- **WebAuthn / FIDO2** – Distinguish between attestation (`webauthn§6.4`, CTAP 2 §5) and assertion (`webauthn§7`):
  ...
  - Signature inspection: record the raw signature as `sig.der.b64u` plus `sig.der.len` (ECDSA) or `sig.raw.b64u`/`sig.raw.len` (RSA/EdDSA) and emit algorithm-specific metadata. For ECDSA algorithms (`ES256`, `ES384`, `ES512`), parse DER into `ecdsa.r.hex` and `ecdsa.s.hex`, compute `ecdsa.lowS`, log `policy.lowS.enforced`, and provide both `valid` and `verify.ok` flags (the latter mirrors the former for new consumers). If policy enforcement is enabled and `ecdsa.lowS=false`, mark the trace as invalid and raise `error.lowS`. For RSA algorithms (`RS256`, `PS256`), emit `rsa.padding`, `rsa.hash`, `key.bits`, and, for PSS, a `rsa.pss.salt.len` attribute. For EdDSA, surface the raw signature hex alongside base64url. Detect discrepancies between descriptor metadata and COSE-derived algorithms, annotate with `error.alg_mismatch`, and stop verification.
