...
|-------------|--------------------------------|
| S24-01 | Core WebAuthn assertion verification engine ingests W3C §16 vectors plus synthetic JSON bundles with deterministic fixtures and failure coverage. |
| S24-02 | Application services, persistence adapters, and telemetry wiring cover stored/inline generation and replay flows with sanitized payloads. |
| S24-03 | CLI and REST facades expose WebAuthn evaluate/replay endpoints, regenerate OpenAPI artifacts, and keep schema/telemetry in sync. |
| S24-04 | Operator console delivers stored/inline generation + replay panels, seed controls, signature counter helpers, and Selenium coverage. |
| S24-05 | Documentation, knowledge map, sample vector catalogues, and seed utilities capture WebAuthn workflows with closing spotless/quality gates. |

## Goals
- Deliver operator workflows for FIDO2/WebAuthn generation and replay, mirroring the REST/CLI endpoints.
- Ensure telemetry, verbose traces, and fixture ingestion match attestation requirements.

## Non-Goals
- Does not extend to WebAuthn authenticator hardware emulation beyond the documented flows.
- Does not modify OCRA/HOTP/TOTP features except where UI scaffolding is shared.
