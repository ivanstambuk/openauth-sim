...
## Goals
- Add replay flows to the OCRA operator UI so stored and inline credentials can be validated visually.
- Keep UI parity with REST/CLI replay APIs, including telemetry/trace toggles.

## Non-Goals
- Does not change evaluation flows beyond the shared components.
- Does not introduce non-OCRA replay flows.
...
|-------------|--------------------------------|
| S16-01 | Selenium/system coverage drives stored and inline replay scenarios, presets, validation errors, and sample removal within the unified console. |
| S16-02 | Replay templates/controllers invoke `/api/v1/ocra/verify` for stored/inline submissions with correct telemetry hooks and result rendering. |
| S16-03 | Telemetry adapters emit replay-specific metadata (mode, outcome, fingerprint) and documentation/runbooks describe the new fields. |
| S16-04 | Follow-up UI cleanups (auto-fill, removal of sample button) plus docs/knowledge map updates complete the user-facing experience. |
