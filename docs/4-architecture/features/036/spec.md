...
|-------------|--------------------------------|
| S36-01 | Core trace tier helper defines `normal`, `educational`, and `lab-secrets` levels with attribute tagging plus exhaustive unit coverage. |
| S36-02 | Application trace builders for HOTP, TOTP, OCRA, and FIDO2 tag every attribute with a minimum tier and filter correctly before returning payloads. |
| S36-03 | CLI, REST, and operator UI facades expose tier-aware payloads/snapshots (tests, OpenAPI, fixtures) so responses match the selected tier. |
| S36-04 | Documentation (spec/plan/how-tos/knowledge map) and roadmap entries describe tier behaviour and note the dependency on future UX toggles. |
| S36-05 | Analysis/quality gates include tier filtering commands to keep regression coverage and spotless builds green once the helper lands. |
