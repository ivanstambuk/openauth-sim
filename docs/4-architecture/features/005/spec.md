...
## Goals
- Deliver Picocli commands for importing, listing, deleting, and evaluating OCRA credentials.
- Mirror REST validation/telemetry semantics so operators get consistent output.

## Non-Goals
- Does not add replay or audit features (tracked separately).
- Does not change REST/UI behaviour beyond ensuring parity.
...
|-------------|--------------------------------|
| S05-01 | `ocra import` persists descriptors through `CredentialStore`, enforces required fields, and prints sanitized confirmations without leaking secrets. |
| S05-02 | `ocra list` / `show` enumerate stored descriptors with redacted metadata so operators can inspect configuration without exposing key material. |
| S05-03 | `ocra delete` removes the targeted descriptor, surfaces success/missing-credential states, and leaves persistence in a consistent state. |
| S05-04 | `ocra evaluate` accepts either stored credential IDs or inline parameters (mutually exclusive) and produces OTPs that match REST/core calculations while surfacing validation errors. |
| S05-05 | All CLI OCRA commands emit sanitized telemetry/logging (event, status, reasonCode) with deterministic exit codes for automation. |
