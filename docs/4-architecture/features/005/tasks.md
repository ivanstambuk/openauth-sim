...
- [x] T-005-73 – Stored credential card configuration visibility (S39-03, FR-005-05): update EMV/CAP operator UI so stored mode masks only EMV keys while rendering card configuration fields (CDOL1, issuer bitmap, ICC payload template, issuer application data) as read-only values in the existing “Card configuration” and “Transaction” fieldsets for both Evaluate and Replay panels; keep inline mode fully editable. Extend Node JS tests ([rest-api/src/test/javascript/emv/console.test.js](rest-api/src/test/javascript/emv/console.test.js)) and Selenium coverage (`EmvCapOperatorUiSeleniumTest`) to assert read-only behaviour and master-key masking, then rerun:
