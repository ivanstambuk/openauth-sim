...
| NFR-014-05 | Facade seam discipline | All facades (CLI/REST/UI/MCP/standalone) consume Native Java APIs by delegating through `application` services, with persistence obtained via `CredentialStoreFactory`; direct use of `core` protocol or persistence internals by facades is prohibited. | ArchUnit facade-boundary tests and contract tests prevent direct `core`/persistence usage. | application, infra-persistence, facades. | Spec. |
| NFR-014-06 | Cross-facade parity baseline | Native Java APIs serve as the canonical behavioural baseline for cross-facade parity. Feature 013 cross-facade contract tests (FR-013-11) compare CLI/REST/UI/standalone results to Native Java outcomes and require parity before facade changes land. | Cross-facade contract suites green; spec updates precede convergence work. | application, cli, rest-api, ui, standalone. | Spec. |
