...
## Goals
- Align module boundaries, shared utilities, and telemetry adapters so protocols follow the same layering conventions.
- Update documentation/diagrams to reflect the harmonized architecture.

## Non-Goals
- Does not create brand-new modules beyond what harmonization requires.
- Does not ship end-user features; it reorganizes existing components.
...
|-------------|--------------------------------|
| S14-01 | Shared `application` services own OCRA orchestration so CLI/REST/UI invoke a single code path for evaluation/replay, including validation/error handling branches. |
| S14-02 | `CredentialStoreFactory` in `infra-persistence` provisions MapDB stores for every facade with configurable paths/in-memory swaps, eliminating duplicate persistence wiring. |
| S14-03 | Telemetry contract interfaces and adapters emit sanitized frames consistently across facades, with shared fixtures covering success/validation/failure events. |
| S14-04 | Unified DTOs/normalisation helpers handle inline vs stored OCRA requests, giving every facade the same field validation and error messaging. |
| S14-05 | `core` splits into `core-shared` and `core-ocra`, and ArchUnit rules enforce that facades depend on application services rather than protocol internals. |
| S14-06 | Documentation (AGENTS, roadmap, knowledge map) and architecture diagrams reflect the new module topology so future protocols follow the same pattern. |
