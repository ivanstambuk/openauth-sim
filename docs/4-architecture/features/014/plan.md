...
| Specification | `docs/4-architecture/features/014/spec.md` |
| Tasks checklist | `docs/4-architecture/features/014/tasks.md` |
...
- **In scope:**
  ...
  - Updating cross-cutting docs (README, docs/2-how-to, `.devin/wiki.json`) to treat Native Java as a facade.
...
| `.devin/wiki.json` | DeepWiki steering config that already lists a Native Java API page. |
...
- **Checklist for future drift-gate runs (agents):**
  ...
  - **Javadoc pipeline verification**  
    ...
    - [ ] `docs/3-reference/native-java-api/README.md` correctly describes:  
    ...
    - [ ] `docs/3-reference/README.md` “Pending artifacts” entry matches the actual behaviour (command + paths).  

  - **Documentation sync & cross-feature alignment**  
    - [ ] README and `docs/2-how-to/README.md` list Native Java as a first-class facade alongside CLI/REST/UI, in line with Feature 014 and ADR‑0007.  
  ...
  - **Drift capture & remediation**  
    - [ ] Any high-/medium-impact drift (e.g., guide names not matching classes, missing Javadoc references to ADR‑0007, broken tests) is:  
      - Logged in `docs/4-architecture/open-questions.md` (Status: `Open`).  
    ...
    - [ ] Low-impact issues (typos, minor Javadoc wording, small doc mismatches) are fixed directly as part of this increment, with:  
      ...
      - A short log entry in `docs/_current-session.md`.  
  ...
  - **Gate output**  
    ...
    - [ ] `docs/_current-session.md` contains a concise recap under the Feature 014 section, including the Javadoc pipeline status and next steps (for example, future Maven publishing or curated HTML publishing when scoped).  
...
1. **I1 – Pattern & documentation alignment (FR-014-01/02)**  
   ...
   - Align README and docs/2-how-to with four consumption surfaces; ensure DeepWiki steering (`.devin/wiki.json`) includes the Native Java API page.  
...
## Analysis Gate
- Run `docs/5-operations/analysis-gate-checklist.md` once increments I1–I3 are in place or when the first protocol (beyond OCRA) is ready to implement its Native Java API.
