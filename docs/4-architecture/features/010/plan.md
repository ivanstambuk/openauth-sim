...
## Increment Map
1. **P3-I1 – Legacy doc suite absorption** (Owner: Ivan, Status: Completed)  
   - Pulled Java/CLI/REST guides + README cross-links from legacy Feature 007 into the consolidated spec; FR-010-01/02 now reference guide paths + telemetry expectations.  
2. **P3-I2 – Quality automation charter** (Owner: Ivan, Status: Completed)  
   - Captured the ArchUnit/Jacoco/PIT/qualityGate workflow plus CI troubleshooting docs from legacy Feature 008 (FR-010-04..09 + NFR updates).  
3. **P3-I3 – Legacy directory cleanup** (Owner: Ivan, Status: Completed)  
   - Removed `docs/4-architecture/features/010/legacy/{007,008}` after verifying spec/plan/tasks coverage; logged the deletion + command outputs in `_current-session.md`.  
4. **P3-I4 – Batch P3 Phase 2 doc gate** (Owner: Ivan, Status: Pending)  
   - Record the cross-feature summary (009–013), rerun `./gradlew --no-daemon spotlessApply check`, and schedule the final `qualityGate` once Features 011–013 complete their rewrites.
