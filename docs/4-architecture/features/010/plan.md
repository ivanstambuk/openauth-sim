...
| Specification | [docs/4-architecture/features/010/spec.md](docs/4-architecture/features/010/spec.md) |
| Tasks checklist | [docs/4-architecture/features/010/tasks.md](docs/4-architecture/features/010/tasks.md) |
| Roadmap entry | #10 – Documentation & Knowledge Automation |

## Vision & Success Criteria
Centralise every operator-facing guide, roadmap/knowledge-map reference, DeepWiki steering config ([.devin/wiki.json](.devin/wiki.json)), and
...
means the Java/CLI/REST guides, README, roadmap, knowledge map, `_current-session.md`, session log ([docs/_current-session.md](docs/_current-session.md)),
...
## Scope Alignment
- Maintain the operator documentation suite (Java/CLI/REST guides, README, roadmap/knowledge map references, `_current-session.md`, session quick reference) and DeepWiki steering config ([.devin/wiki.json](.devin/wiki.json)) inside Feature 010’s spec/plan/tasks so these artefacts evolve together.
- Keep roadmap, knowledge map, architecture graph, DeepWiki page outline, session log ([docs/_current-session.md](docs/_current-session.md)), and session quick reference aligned with documentation and automation changes.
- Maintain the aggregated `qualityGate` task plus its CI workflow, report locations, skip flags, and troubleshooting guides.
- Log every documentation/automation increment—including changes to [.devin/wiki.json](.devin/wiki.json)—inside `_current-session.md` with command history.
- Enforce the Option A documentation-link policy (NFR-010-06): in all human-facing Markdown (README/AGENTS, `docs/0-overview`…8-compliance, specs/plans/tasks, runbooks, roadmap, knowledge map, `_current-session.md`), link repo files via `[path](path)`, keep directories as inline code/plain text, and leave fenced code and multi-line commands raw. Capture deviations in `_current-session.md` and plan future sweeps/automation here.
- Add enforcement automation where feasible (for example, a pre-commit Markdown newline guard) so NFR-010-06 stays compliant without recurring manual sweeps; track these hooks/scripts in this feature’s plan/tasks.
...
| Roadmap / knowledge map / session log ([docs/_current-session.md](docs/_current-session.md)) / `_current-session.md` | Need synchronized references and command logs per increment. |
...
- Risk: forgetting to log commands in `_current-session.md` or session log ([docs/_current-session.md](docs/_current-session.md)) reduces auditability—mitigate via checklist items.
- Risk: PIT/Jacoco runtimes can exceed NFR limits if cache hints regress—monitor timings inside tasks/plan notes.

## Implementation Drift Gate

- Summary: Use this gate to ensure the docs/automation surface (operator guides, protocol reference pages, README, external-dependency reference, roadmap/knowledge map, session logs, DeepWiki steering, `qualityGate` and CI wiring, and LLM/AI-assistant oriented docs) stays aligned with FR-010-01..13 and NFR-010-01..05, and that every change to documentation or automation is reflected consistently across artefacts and logged in `_current-session.md`.
...
- **Checklist for future drift-gate runs (agents):**
  - **Preconditions**
    ...
    - [ ] [docs/4-architecture/open-questions.md](docs/4-architecture/open-questions.md) has no `Open` entries for Feature 010.  
  ...
  - **Spec ↔ docs/automation mapping**
    - [ ] For FR-010-01..FR-010-13 and NFR-010-01..05, confirm the spec’s expectations are reflected in:  
      ...
      - [README.md](README.md) and [docs/2-how-to/README.md](docs/2-how-to/README.md) landing pages.  
      - LLM/agent-oriented artefacts ([ReadMe.LLM](ReadMe.LLM), [llms.txt](llms.txt), AGENTS cross-links) that describe canonical Native Java entry points (Feature 014) and protocol coverage.  
      - External dependency matrix reference ([docs/3-reference/external-dependencies-by-facade-and-scenario.md](docs/3-reference/external-dependencies-by-facade-and-scenario.md)).  
      - Roadmap ([docs/4-architecture/roadmap.md](docs/4-architecture/roadmap.md)) and knowledge map ([docs/4-architecture/knowledge-map.md](docs/4-architecture/knowledge-map.md)).  
      - Session quick reference ([docs/5-operations/session-quick-reference.md](docs/5-operations/session-quick-reference.md)).  
      - Session log ([docs/_current-session.md](docs/_current-session.md)) and any related logs referenced in the spec.  
    - [ ] Ensure [.devin/wiki.json](.devin/wiki.json) mirrors the current docs structure and feature catalogue (including the Feature 010 role) and that DeepWiki descriptions match the spec’s view of docs/automation and LLM/assistant-facing responsibilities.  
  ...
  - **Automation & quality-gate alignment**
    - [ ] `qualityGate` definition in [build.gradle.kts](build.gradle.kts) still matches the spec:  
    ...
    - [ ] [.github/workflows/ci.yml](.github/workflows/ci.yml) invokes `./gradlew --no-daemon qualityGate` (or the documented equivalent) and uploads or references reports as described in the spec/plan.  
  ...
  - **Governance & logging**
    - [ ] Feature 011 hook/constitution requirements referenced from this spec are still honoured:  
      - `git config core.hooksPath` logs appear in [docs/_current-session.md](docs/_current-session.md).  
      - Pre-commit and commit-msg hooks enforce Spotless, tests, secret scanning, and gitlint as described.  
    - [ ] Every documentation/automation increment has a corresponding entry in [docs/_current-session.md](docs/_current-session.md) that mentions:  
  ...
  - **Drift capture & remediation**
    - [ ] Any high-/medium-impact drift (e.g., guides contradicting behaviour, `qualityGate` behaviour diverging from docs, missing logging of key commands) is:  
      - Logged as an `Open` row in [docs/4-architecture/open-questions.md](docs/4-architecture/open-questions.md) for Feature 010.  
      - Captured as explicit tasks in [docs/4-architecture/features/010/tasks.md](docs/4-architecture/features/010/tasks.md) (and, if cross-cutting, in other features’ plans).  
  ...
  - **Gate output**
    ...
    - [ ] [docs/_current-session.md](docs/_current-session.md) logs that the Feature 010 Implementation Drift Gate was executed (date, commands, and reference to this plan section).  
...
| S-010-03 | Roadmap/knowledge map/session log ([docs/_current-session.md](docs/_current-session.md))/session quick reference stay synchronized and log commands. | P3-I1–P3-I4 |
...
| S-010-10 | `_current-session.md` + session log ([docs/_current-session.md](docs/_current-session.md)) log every documentation/automation increment. | P3-I3 |

## Analysis Gate
Run [docs/5-operations/analysis-gate-checklist.md](docs/5-operations/analysis-gate-checklist.md) whenever major doc/automation scope changes land. Ensure roadmap, knowledge map, session log ([docs/_current-session.md](docs/_current-session.md)), and `_current-session.md` match the updated spec/plan/tasks before moving to implementation work.

## Exit Criteria
- Feature 010 spec/plan/tasks describe the operator doc suite and quality automation guardrails (no `legacy/` references).
- Roadmap, knowledge map, architecture graph, session log ([docs/_current-session.md](docs/_current-session.md)), session quick reference, and `_current-session.md` cite Feature 010 for documentation and automation work.
...
- Track future AI/agent integration work (for example Maven publication metadata, MCP server modules, JSON-LD/Schema.org metadata for hosted docs, and other Option C GEO additions) as follow-up items coordinated with Feature 013 once the initial LLM/assistant-facing documentation surfaces from FR-010-13 are stable. Maven-related work remains parked until public coordinates are confirmed; when that happens, execute T-010-07 to wire the coordinates into docs and enrich POM metadata.
