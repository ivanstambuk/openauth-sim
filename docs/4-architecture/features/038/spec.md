...
## Goals
- Show ordered OTP preview tables in evaluation results across REST, CLI, and operator UI.
- Replace evaluation drift inputs with preview-window controls and ensure telemetry traces capture the chosen window.

## Non-Goals
- Does not modify replay flows (drift inputs remain there).
- Does not change persistence or credential seeding.
...
These ASCII sketches cover the operator console result card changes and are referenced by Selenium tests.
...
|-------------|--------------------------------|
| S38-01 | REST HOTP/TOTP/OCRA evaluation endpoints accept `window.backward`/`window.forward`, default missing values to zero, remove evaluation drift fields, and return ordered preview rows with the evaluated Δ = 0 entry always present. |
| S38-02 | Application services and CLI commands propagate preview windows, drop evaluation drift flags, and render preview tables (text + JSON) with telemetry parity across all protocols. |
| S38-03 | Operator UI evaluation panels expose preview window controls, render the result-card table (stored and inline) with protocol-coloured Δ = 0 accent bars, and cover offsets {0,0} and {2,4}. |
| S38-04 | Accessibility, telemetry, and documentation stay aligned: preview rows remain perceivable without colour, how-to guides/roadmap entries reflect the feature, and traces capture window selections. |
| S38-05 | Preview window controls replace helper copy on HOTP/TOTP/OCRA evaluation forms, keeping the layout concise while retaining drift inputs exclusively on replay flows. |
