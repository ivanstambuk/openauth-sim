...
|-------------|--------------------------------|
| S35-01 | Core trace model and builders emit ordered, immutable verbose traces with full cryptographic payloads for every supported protocol. |
| S35-02 | Application services propagate per-request verbose toggles and produce traces for HOTP, TOTP, OCRA, and WebAuthn evaluate/replay/attest flows without affecting non-verbose behaviour. |
| S35-03 | CLI and REST facades honor verbose flags/fields, returning the same trace content (JSON + text) that the application layer produces. |
| S35-04 | Operator UI surfaces verbose traces through the shared console panel, enabling copy/download actions while keeping stored vs inline journeys in sync. |
| S35-05 | WebAuthn-specific traces capture canonical RP IDs, signature components, COSE metadata, flag maps, and match indicators with deterministic formatting. |
| S35-06 | Documentation, telemetry catalogues, knowledge map, and roadmap entries describe verbose tracing behaviour and stay aligned as new trace fields ship. |
