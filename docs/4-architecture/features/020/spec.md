...
## Goals
- Introduce protocol tabs in the operator UI so HOTP/TOTP/OCRA flows share the same page shell.
- Ensure navigation, theming, and telemetry toggles behave consistently across tabs.

## Non-Goals
- Does not add new protocol implementations.
- Does not modify REST/CLI behaviour except where UI requires parity.
...
|-------------|--------------------------------|
| S20-01 | Operator console renders HOTP → TOTP → OCRA → EMV/CAP → FIDO2/WebAuthn → EUDIW OpenID4VP → EUDIW ISO/IEC 18013-5 → EUDIW SIOPv2 tabs with consistent placeholder panels and accessibility attributes. |
| S20-02 | Query-parameter routing/history recognise the new protocol keys so deep links restore the correct tab state and navigation remains consistent. |
| S20-03 | Roadmap/knowledge map entries enumerate HOTP, TOTP, and EUDI wallet workstreams with updated status, replacing the legacy catch-all milestone. |
