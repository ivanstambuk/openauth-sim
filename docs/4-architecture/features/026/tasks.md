...
_Linked plan:_ `docs/4-architecture/features/026/plan.md`  
_Status:_ Complete (awaiting acceptance)  
_Last updated:_ 2025-11-10

> Keep this checklist aligned with the plan. Stage tests before implementation, record verification commands, and retain historical detail below for auditability.

## Checklist
- [x] T-026-01 – Fixture scaffolding & verifier baselines (S-026-01).  
  _Intent:_ Import W3C + synthetic attestation fixtures and stage failing generation/verification suites before the core helpers landed; supersedes legacy T2601–T2608.  
  _Verification commands:_  
  - `./gradlew --no-daemon :core:test --tests "io.openauth.sim.core.fido2.WebAuthnAttestationFixturesTest"`  
  - `./gradlew --no-daemon :core:test --tests "io.openauth.sim.core.fido2.WebAuthnAttestationVerifierTest"`  
  - `./gradlew --no-daemon spotlessApply check`  
  _Notes:_ See Legacy Task Log §Foundation for per-task notes.

- [x] T-026-02 – Manual generation + CLI/UI parity (S-026-03, S-026-04).  
  _Intent:_ Wire manual mode detection, CLI options, and operator UI overrides before Stored mode; covers legacy T2609–T2622.  
  _Verification commands:_  
  - `node --test rest-api/src/test/javascript/fido2/*.test.js`  
  - `./gradlew --no-daemon :rest-api:test --tests "io.openauth.sim.rest.fido2.*Attestation*Test"`  
  - `./gradlew --no-daemon :cli:test --tests "io.openauth.sim.cli.Fido2CliAttestationTest"`  
  _Notes:_ Details under Legacy Task Log §Manual Mode.

- [x] T-026-03 – Replay, trust anchors, and telemetry (S-026-02).  
  _Intent:_ Deliver replay endpoints, curated/manual trust-anchor selectors, and telemetry snapshots prior to stored credentials; maps to T2623–T2639.  
  _Verification commands:_  
  - `./gradlew --no-daemon :application:test --tests "*WebAuthnAttestationServiceTest"`  
  - `./gradlew --no-daemon :rest-api:test --tests "io.openauth.sim.rest.fido2.*Replay*Test"`  
  - `./gradlew --no-daemon pmdMain pmdTest`  
  _Notes:_ See Legacy Task Log §Replay & Telemetry for task-level outcomes.

- [x] T-026-04 – Stored credential seeding & sanitisation (S-026-05).  
  _Intent:_ Seed curated stored credentials, hydrate MapDB workflows, and sanitize outputs before the drift-gate snapshot; spans T2640–T2650.  
  _Verification commands:_  
  - `./gradlew --no-daemon :application:test :cli:test :rest-api:test :ui:test pmdMain pmdTest spotlessApply check`  
  - `OPENAUTH_SIM_PERSISTENCE_DATABASE_PATH=build/tmp/test-credentials.db ./gradlew --no-daemon :rest-api:test --tests "io.openauth.sim.rest.ui.Fido2AttestationOperatorUiSeleniumTest"`  
  _Notes:_ Legacy Task Log §Stored Credentials captures seeding/sanitisation history.

- [x] T-026-05 – Documentation, telemetry, and drift-gate evidence (S-026-05).  
  _Intent:_ Record roadmap/how-to updates, telemetry snapshots, and the 2025-11-05 Implementation Drift Gate outcome covering stored credential sanitisation (T2650).  
  _Verification commands:_  
  - `./gradlew --no-daemon spotlessApply check`  
  - `./gradlew --no-daemon :application:test :cli:test :rest-api:test :ui:test pmdMain pmdTest` (gate confirmation)  
  _Notes:_ Evidence archived in docs/_current-session.md and migration tracker; references legacy verification log entries below.

## Verification Log
- 2025-11-05 – `./gradlew --no-daemon :application:test :cli:test :rest-api:test :ui:test pmdMain pmdTest spotlessApply check`
- 2025-10-19 – `./gradlew --no-daemon :core:test --tests "io.openauth.sim.core.fido2.WebAuthnAttestationVerifierTest"`
- 2025-11-10 – `./gradlew --no-daemon spotlessApply check` (template migration validation)

## Notes / TODOs
- Awaiting owner acceptance of stored-mode UX; no open follow-ups.
- Legacy task-by-task notes remain below for forensic review.

## Legacy Task Log
> The content below preserves the pre-template checklist (T2601–T2650) without modification.

_Linked plan:_ `docs/4-architecture/features/026/plan.md`  
_Status:_ Complete  
_Last updated:_ 2025-11-05 (T2650 stored credential sanitisation complete) (S-026-05)

☑ **T2601 – Fixture scaffolding**   (S-026-01)
...
☑ **T2602 – Core attestation engine implementation**   (S-026-01)
 ☑ Implement attestation helpers satisfying T2601 tests, including format-specific validation and error mapping.   (S-026-01)
...
☑ **T2603 – Application services & telemetry**   (S-026-02)
...
☑ **T2606 – Operator UI Evaluate toggle**   (S-026-04)
...
☑ **T2607 – Operator UI Replay attestation support**   (S-026-04)
...
☑ **T2608 – Fixture ingestion & documentation**   (S-026-04)
...
☑ **T2609 – Trust anchor ingestion helpers**   (S-026-04)
...
☑ **T2610 – MDS scaffolding**   (S-026-01)
...
☑ **T2611 – Generator test scaffolding**   (S-026-01)
...
☑ **T2612 – Core generator implementation**   (S-026-01)
...
☑ **T2613 – Facade integration (CLI/REST)**   (S-026-03)
...
☑ **T2614 – Operator UI Evaluate migration**   (S-026-04)
...
☑ **T2615 – Checkstyle/PMD remediation**   (S-026-02)
...
☑ **T2616 – Attestation result panel trim**   (S-026-04)
...
☑ **T2617 – Attestation response payload reduction**   (S-026-04)
...
☑ **T2623 – Credential ID alignment**   (S-026-03)
...
☑ **T2625 – UI certificate chain count badge**   (S-026-04)
 ☑ Reintroduce the attestation result certificate-chain count label (matching the pre-T2617 summary styling) while keeping the PEM block separate from the JSON payload.   (S-026-04)
...
☑ **T2626 – UI attestation result layout**   (S-026-04)
...
☑ **T2627 – Certificate heading styling**   (S-026-04)
...
☑ **T2629 – Assertion payload sanitization**   (S-026-03)
...
☑ **T2628 – Attestation private key format parity**   (S-026-04)
...
☑ **T2618 – Core Manual input source**   (S-026-02)
...
☑ **T2619 – Application service Manual wiring**   (S-026-02)
...
☑ **T2620 – REST inputSource + conditional validation**   (S-026-03)
...
☑ **T2621 – UI auto-switch + Manual mode**   (S-026-04)
...
☑ **T2622 – CLI parity for Manual**   (S-026-03)
...
☑ **T2630 – Attestation preset label harmonisation**   (S-026-04)
...
☑ **T2631 – Credential store attestation schema**   (S-026-05)
...
☑ **T2632 – Stored attestation generation services**   (S-026-05)
...
☑ **T2633 – CLI/REST stored flow support**   (S-026-03)
...
☑ **T2634 – Operator UI stored selector**   (S-026-04)
...
☑ **T2635 – Seed stored attestation credentials**   (S-026-05)
...
☑ **T2636 – Stored attestation replay test scaffolding**   (S-026-05)
...
☑ **T2637 – Stored attestation replay implementation**   (S-026-05)
	☑ Implement REST `inputSource=STORED` replay handling (DTO validation, service/controller branches), CLI flag plumbing, and telemetry enrichment mirroring stored generation reason codes.  
		...
		- UI: align with stored replay toggle (covered under T2638) but ensure API responds with stored challenge/clientData/attestationObject to hydrate Selenium expectations.   (S-026-04)
	☑ Drive T2636 tests green, refresh OpenAPI snapshots, and document any new reason codes.   (S-026-05)
...
☑ **T2638 – Operator UI stored replay & identifier removal**   (S-026-04)
...
☑ **T2639 – Documentation & telemetry sync**   (S-026-05)
...
☑ **T2640 – Attestation replay sample loader parity**   (S-026-04)
...
☑ **T2641 – Stored replay payload display**   (S-026-04)
...
☑ **T2642 – Stored replay challenge hydration fix**   (S-026-04)
...
☑ **T2643 – Stored attestation label normalization**   (S-026-04)
...
☑ **T2644 – Unified stored credential seeding**   (S-026-05)
...
☑ **T2645 – Synthetic PS256 attestation fixture**   (S-026-05)
...
☑ **T2646 – Stored replay dropdown stabilisation**   (S-026-04)
...
☑ **T2647 – Stored replay read-only styling parity**   (S-026-04)
...
☑ **T2648 – Curated trust anchor selector**   (S-026-04)
...
☑ **T2649 – Stored replay trust-anchor summary**   (S-026-05)
...
- [x] **T2650 – Stored credential secret sanitisation**   (S-026-05)
...
- 2025-11-05 – Sanitisation follow-up (T2650) completed; owner to confirm re-closure post-review. (S-026-05)
