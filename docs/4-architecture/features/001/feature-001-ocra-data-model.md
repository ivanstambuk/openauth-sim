...
## Goals
- Document the canonical OCRA data model (suites, secret material, counter/session fields) for use across facades and persistence layers.
- Capture validation rules, defaults, and serialization formats so specifications, plans, and tasks remain aligned.

## Non-Goals
- Does not define OTP execution helpers or RFC 6287 algorithms (covered by core implementation features).
- Does not add user-facing APIs or UI pages; this is a persistence/data-contract reference.
