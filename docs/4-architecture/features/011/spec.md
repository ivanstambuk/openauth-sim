...
## Goals
- Document and enforce the no-reflection policy across codebases, adding guards/tests where needed.
- Provide guidance for exposing seams without reflection for tests and instrumentation.

## Non-Goals
- Does not ban legitimate framework reflection already vetted (e.g., Spring).
- Does not refactor unrelated modules beyond enforcing the policy.
...
|-------------|--------------------------------|
| S11-01 | All project-owned production and test code replaces reflection with explicit seams, and repository scans confirm zero `java.lang.reflect.*` usage outside documented exemptions. |
| S11-02 | `core-architecture-tests` ArchUnit guard fails builds when reflection classes are imported, ensuring automation blocks regressions before merge. |
| S11-03 | Gradle/regex guard (`reflectionScan`) scans source artifacts and build scripts for banned reflection markers, keeping the quality gate red until the offending code is removed or approved. |
| S11-04 | AGENTS.md and related contributor docs capture the anti-reflection policy plus mitigation strategies so future work aligns without rereading the entire spec. |
| S11-05 | CLI/REST/core seams added during the refactor are exercised by updated unit/integration tests, proving behaviour parity without reflection. |
