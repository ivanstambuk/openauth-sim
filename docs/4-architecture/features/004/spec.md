...
## Goals
- Add REST APIs for listing, resolving, and deleting OCRA credentials with sanitized payloads.
- Ensure credential resolution honours MapDB persistence rules and telemetry policies.

## Non-Goals
- Does not introduce new credential types beyond OCRA.
- Does not alter CLI or UI credential management flows.

## Branch & Scenario Matrix
| Scenario ID | Description / Expected outcome |
|-------------|--------------------------------|
| S04-01 | Requests referencing `credentialId` load descriptors from persistence and evaluate OTPs without requiring inline secrets. |
| S04-02 | Inline secret submissions remain fully supported when `credentialId` is absent, preserving prior behaviour. |
| S04-03 | Validation enforces mutual exclusivity between modes and returns descriptive reason codes for conflicts or missing credentials. |
| S04-04 | Telemetry/logging capture `hasCredentialReference` plus lookup failures without exposing sensitive data. |
| S04-05 | OpenAPI, operator docs, and telemetry snapshots reflect the dual-mode contract and new error codes. |
