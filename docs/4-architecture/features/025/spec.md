...
## Goals
- Standardize sample vector labels, dropdowns, and hints across all operator UI panels.
- Keep Selenium coverage enforcing the shared copy and layout.

## Non-Goals
- Does not introduce new sample data; only naming/UX alignment.
- Does not modify REST/CLI outputs.
...
|-------------|--------------------------------|
| S25-01 | HOTP/TOTP/OCRA/FIDO2 operator-console dropdowns adopt the concise `<scenario – attributes>` label pattern (no "sample vector" wording) with updated hints/placeholders and refreshed tests/docs. |
| S25-02 | HOTP seeded credentials and inline presets expand to cover SHA-1/256/512 across 6- and 8-digit variants so stored/inline catalogs stay aligned. |
| S25-03 | RFC 6287 labels append `(RFC 6287)` for both inline and stored OCRA presets while draft-only entries keep their original names. |
| S25-04 | Verification artifacts (Selenium, docs, roadmap/session notes) and the final spotlessApply check confirm the harmonised labels ship without regressions. |
