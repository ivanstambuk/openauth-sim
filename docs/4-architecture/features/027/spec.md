...
|-------------|--------------------------------|
| S27-01 | Governance artefacts (spec/plan/tasks, roadmap, knowledge map) capture the unified `credentials.db` decision. |
| S27-02 | `CredentialStoreFactory` and related helpers always default to `credentials.db` with updated tests/logging. |
| S27-03 | CLI/REST facades, tests, and Selenium coverage adopt the shared default filename and pass regression builds. |
| S27-04 | Documentation/how-to guides describe the migration path and note that legacy filenames require manual renaming. |
