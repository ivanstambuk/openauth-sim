...
## Goals
- Harden the MapDB + Caffeine stack for OCRA credential storage with encryption, compaction, and performance baselines.
- Expose instrumentation/telemetry so persistence behaviour can be monitored across facades.

## Non-Goals
- Does not replace MapDB with a new database engine.
- Does not alter higher-level credential APIs beyond what persistence requires.

## Branch & Scenario Matrix
| Scenario ID | Description / Expected outcome |
|-------------|--------------------------------|
| S02-01 | Deployment profiles (in-memory, file-backed, container) expose tuned cache defaults with documented override hooks for builders. |
| S02-02 | Structured metrics and diagnostics report cache hit/miss, latency, and maintenance events without leaking secrets. |
| S02-03 | Maintenance helper executes compaction/integrity operations and surfaces structured results plus CLI entry points. |
| S02-04 | Optional at-rest protection hooks (encryption provider interface) guard MapDB payloads while leaving the default configuration unchanged. |
| S02-05 | Benchmark harness demonstrates ≥10k RPS throughput with recorded P99 latencies, ensuring cache tuning meets targets. |
| S02-06 | Documentation/roadmap capture configuration guidance, maintenance instructions, and encryption usage for operators. |
