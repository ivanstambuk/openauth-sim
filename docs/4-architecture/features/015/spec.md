...
## Goals
- Enable SpotBugs dead-state rules and remediate flagged issues across modules.
- Ensure build pipelines fail when new dead-state regressions appear.

## Non-Goals
- Does not replace SpotBugs with another tool.
- Does not alter runtime logic except where required to satisfy the rules.
...
|-------------|--------------------------------|
| S15-01 | Shared SpotBugs include filter plus Gradle wiring enables dead-state detectors across all JVM modules. |
| S15-02 | Existing dead-state violations are remediated, keeping SpotBugs dead-state tasks green. |
| S15-03 | Documentation/runbooks/analysis gate notes explain the new enforcement and suppression policy. |
| S15-04 | PMD rules `UnusedPrivateField` / `UnusedPrivateMethod` join the quality gate with code cleanup completed. |
