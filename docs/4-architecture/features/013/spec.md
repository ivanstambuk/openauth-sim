...
| FR-013-13 | Facade boundary enforcement MUST be centralised: a single ArchUnit suite defines canonical facade package constants and reusable boundary rules (CLI↔REST bans, MCP↔REST bans, etc.) to avoid duplicated, drift-prone checks. | `core-architecture-tests` includes a single `FacadeBoundariesArchitectureTest` that owns facade package constants and rule helpers; other ArchUnit tests reference those constants/rules. | `./gradlew --no-daemon :core-architecture-tests:test` and `./gradlew --no-daemon spotlessApply check`. | Boundary rules become scattered and inconsistent, slowing future enforcement work. | None. | Spec. |
