...
## Goals
- Leverage Java 17 language/library features to simplify code while keeping behaviour identical.
- Update build tooling/settings so Java 17 becomes the enforced baseline.

## Non-Goals
- Does not raise the minimum version beyond Java 17.
- Does not rewrite protocols or architecture—changes are mechanical/refinement focused.
...
|-------------|--------------------------------|
| S13-01 | `OcraCli` command hierarchy becomes sealed, and CLI tests confirm only the permitted Picocli subcommands execute, preserving existing behaviour without reflection. |
| S13-02 | REST OCRA evaluation/verification normalization relies on sealed request variants plus pattern matching so stored vs inline flows share the same canonical DTOs. |
| S13-03 | OpenAPI example payloads migrate to Java text blocks while serialization snapshots remain unchanged for operators and docs. |
| S13-04 | Quality gates (spotless, Jacoco, ArchUnit, PIT) stay green after the Java 17 refactors, proving the language upgrades do not erode automation coverage. |
