...
|-------------|--------------------------------|
| S31-01 | CLI flows emit telemetry exclusively through `TelemetryContracts` adapters; legacy `legacyEmit` branches and tests are removed. |
| S31-02 | Operator console routing/query-state shims (`__openauth*`, legacy param aliases) are deleted so navigation relies on the canonical protocol/tab/mode keys. |
| S31-03 | Console networking and ceremony bridges drop XMLHttpRequest/fetch polyfills and FIDO2 `legacySetMode`, relying solely on the modern fetch-based helpers. |
| S31-04 | WebAuthn generator legacy presets are pruned, documentation/knowledge artefacts updated, and the analysis gate re-run to confirm the cleansed surface. |
