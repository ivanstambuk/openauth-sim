...
## Goals
- Produce operator-facing documentation explaining how to run CLI, REST, and UI flows for the shipped protocols.
- Ensure guides capture telemetry expectations, preset seeding, and trace toggles.

## Non-Goals
- Does not change runtime behaviour; it is documentation-only.
- Does not cover future protocols beyond those explicitly in scope.

## Branch & Scenario Matrix
| Scenario ID | Description / Expected outcome |
|-------------|--------------------------------|
| S07-01 | Java operator integration guide demonstrates how external applications call simulator helpers or REST/CLI bridges with runnable snippets. |
| S07-02 | CLI operations guide documents every shipped command (import/list/delete/evaluate/maintenance) with copy-ready examples and troubleshooting tips. |
| S07-03 | REST operations guide consolidates all endpoints, schemas, and Swagger UI usage so operators can execute flows without referencing code. |
| S07-04 | README reflects current simulator capabilities, adds the Swagger UI link, and removes references to unimplemented interfaces. |
| S07-05 | Documentation set cross-links consistently and updates roadmap/knowledge map entries after publication. |
