...
## Current Links
- Feature 011 is now the sole governance source of truth. Its spec/plan/tasks (`docs/4-architecture/features/011/{spec,plan,tasks}.md`) catalogue FR-011-01..08 and NFR-011-01..05, describe AGENTS/runbook/constitution ownership, spell out gitlint + Palantir formatter policies, and require logging every `git config core.hooksPath`, `githooks/pre-commit`, and `./gradlew --no-daemon spotlessApply check`/`qualityGate` invocation in `_current-session.md` plus `docs/migration_plan.md`.
...
- Feature 009 now aggregates all operator-console UI/JS work; the legacy Features 017/020/021/025/033–038/041 specs/plans/tasks reside under `docs/4-architecture/features/009/legacy/` while the consolidated spec documents REST/CLI/UI linkages and verbose trace dependencies.
- Feature 010 now hosts documentation and knowledge automation artefacts, keeping legacy operator how-to suites (former Features 007/008) under `docs/4-architecture/features/010/legacy/` so roadmap, knowledge map, and session quick reference updates share the same source.
...
- Renumbering Batch P1 (2025-11-11) moved HOTP/TOTP/OCRA work into Features 001/002/003, with legacy Feature 001/003/004/009/016/018 artifacts preserved under `docs/4-architecture/features/003/legacy/` for auditability while the new specs own the authoritative requirements.
- Renumbering Batch P2 (2025-11-11) reassigned WebAuthn + EMV/CAP + EUDIW documentation: Feature 004 now embeds the
  former Feature 024/026 specs inline, Feature 005 carries the entire Feature 039 EMV/CAP stack, Feature 006 carries the
  former Feature 040 OpenID4VP spec, and Features 007/008 remain placeholders for the upcoming mdoc/SIOPv2 simulators while
  the migrated doc/quality artefacts now live under `docs/4-architecture/features/010/legacy/007` and
  `docs/4-architecture/features/010/legacy/008` inside Feature 010’s ownership. The copied
  Feature 024/026/039/040 directories can now be deleted after spotless + targeted REST/UI verification.
- Feature 004 now hosts the consolidated Feature 024/026 content in a template-friendly spec/plan/tasks set whose FR/NFR/S/T
  entries call out assertion evaluation, attestation generation, telemetry, and documentation handoffs; the session log (docs/_current-session.md),
  knowledge map, and `_current-session.md` note the rewrite plus the verification commands that closed the Batch P2 gate.
