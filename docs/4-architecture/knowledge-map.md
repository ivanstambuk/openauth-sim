...
- CLI module now provides `Fido2Cli` with stored/inline evaluate, replay, and attestation commands (verify + replay) that delegate to the corresponding application services, reuse sanitized telemetry fields, and trim attestation generation output to `clientDataJSON`/`attestationObject` while emitting signature/certificate statistics via telemetry instead of raw PEM payloads.
...
- Operator UI Replay tab now mirrors attestation verification: the ceremony toggle locks replay into inline-only mode, the form accepts Base64URL payloads plus optional PEM trust anchors, and `console.js` surfaces sanitized telemetry/errors from `/api/v1/webauthn/attest/replay` into the Selenium-observed result panel.
