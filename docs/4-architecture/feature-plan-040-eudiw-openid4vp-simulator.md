...
7. **S7 – Operator UI integration (F-040-17/21/24/25/29/30)**  
   ...
   - _Next 2025-11-09 (T4016):_ Stage failing JS + Selenium assertions for multi-presentation rendering (collapsible cards, descriptor labels, copy/download buttons, shared `data-trace-id`), extend `OpenId4VpValidationServiceTest` (and wallet simulators if needed) to emit multiple presentation payloads, then implement the UI/controller wiring so each presentation section maps to its verbose trace entry. Finish with the targeted commands listed in the task checklist plus a full `./gradlew --no-daemon spotlessApply check`.
    ...
    - _In flight 2025-11-09 (T4018 – Deep-link & flag consolidation):_ Scope covers two threads before S8 begins. (1) Treat `protocol=eudiw` as a first-class alias so query parameters (`tab`, `mode`) hydrate Evaluate/Replay—including history navigation—by wiring the global console + EUDIW JS to parse/apply URL state and push replacements when operators change radio buttons. Add JS unit tests to cover alias parsing, tab/mode hydration, and history sync plus Selenium coverage that loads `/ui/console?protocol=eudiw&tab=replay&mode=stored` and asserts the Replay/Stored sections display. (2) Align REST/CLI verbose traces by introducing a shared `Oid4vpVerboseTraceBuilder` (application layer) that converts simulator traces into the canonical `VerboseTrace` shape, updating the REST controller/OpenAPI responses to emit `VerboseTracePayload` instances under `trace` and teaching the Picocli commands to honor `--verbose/--no-verbose` by printing the standard warning + payload (JSON includes the payload; console output routes through `VerboseTracePrinter`). Commands: `node --test rest-api/src/test/javascript/eudi/openid4vp/console.test.js`, `OPENAPI_SNAPSHOT_WRITE=true ./gradlew --no-daemon :rest-api:test --tests "io.openauth.sim.rest.OpenApiSnapshotTest"`, `OPENAUTH_SIM_PERSISTENCE_DATABASE_PATH=build/tmp/test-credentials.db ./gradlew --no-daemon :rest-api:test --tests "io.openauth.sim.rest.ui.EudiwOperatorUiSeleniumTest"`, `./gradlew --no-daemon :rest-api:test --tests "io.openauth.sim.rest.eudi.openid4vp.Oid4vpRestContractTest"`, `./gradlew --no-daemon :cli:test --tests "io.openauth.sim.cli.eudi.openid4vp.Oid4vpCliContractTest"`, and the closing `./gradlew --no-daemon spotlessApply check` (expect ≥600 s when Selenium + snapshot + verbose trace wiring run together).
