...
The `qualityGate` Gradle task aggregates the architectural, coverage, and mutation checks that protect the OCRA stack. Run it locally before shipping changes touching `core`, `cli`, or `rest-api`, and rely on the CI workflow ([.github/workflows/ci.yml](.github/workflows/ci.yml)) for enforcement on every push and pull request. Feature 013 (``docs/4-architecture/features/013`/{spec,plan,tasks}.md`) is the canonical source for toolchain and quality automation requirements; keep this guide aligned with its FR/NFR set.

## Commands
- **Format before commit:** `./gradlew spotlessApply`
  - Runs Palantir Java Format 2.78.0 via Spotless across all Java sources so contributors (human or AI) stage diffs that already satisfy the gate.
...
## Troubleshooting
- **Coverage dips below threshold:** inspect [build/reports/jacoco/aggregated/html/index.html](build/reports/jacoco/aggregated/html/index.html) to locate untested lines. Add or adjust tests, then rerun `qualityGate`.
...
## SpotBugs Dead-State Detectors (Feature 013)
- Every `spotbugsMain` task now applies [config/spotbugs/dead-state-include.xml](config/spotbugs/dead-state-include.xml), which promotes the `URF_UNREAD_FIELD`, `URF_UNREAD_PUBLIC_OR_PROTECTED_FIELD`, `UUF_UNUSED_FIELD`, `UWF_UNWRITTEN_FIELD`, and `NP_UNWRITTEN_PUBLIC_OR_PROTECTED_FIELD` patterns to hard failures.
...
- PMD now enforces `UnusedPrivateField` (best practices category) via [config/pmd/ruleset.xml](config/pmd/ruleset.xml). Run `./gradlew :<module>:pmdMain` or `:pmdTest` to surface unused private members—remove the dead field or justify it before re-running `qualityGate`.
