...
final class HotpOperatorUiSeleniumTest {
    ...
    private static final HotpDescriptor INLINE_SHA256_DESCRIPTOR = HotpDescriptor.create(
    ...
    void storedCredentialEvaluationSucceeds() {
        ...
        WebElement storedToggle = modeToggle.findElement(By.cssSelector("[data-testid='hotp-mode-select-stored']"));
        WebElement inlineToggle = modeToggle.findElement(By.cssSelector("[data-testid='hotp-mode-select-inline']"));
        ...
        if (inlineToggle.isSelected()) {
            throw new AssertionError("Inline HOTP mode toggle should be deselected after stored activation");
        }

        WebElement storedPanel = new WebDriverWait(driver, Duration.ofSeconds(5))
        ...
        if (inlinePanel.isDisplayed()) {
            throw new AssertionError("Inline HOTP evaluation panel should hide once stored mode is active");
        }
        ...
        driver.findElement(By.cssSelector("button[data-testid='hotp-stored-evaluate-button']"))
                .click();
    ...
    void hotpInlineSpacingMatchesOcra() {
        ...
        WebElement presetLabel = new WebDriverWait(driver, Duration.ofSeconds(5))
                .until(ExpectedConditions.visibilityOfElementLocated(
    ...
    void sampleVectorLabelAlignsWithReplay() {
        ...
        WebElement evaluateHeading = new WebDriverWait(driver, Duration.ofSeconds(5))
                .until(ExpectedConditions.visibilityOfElementLocated(
    ...
    void hotpPresetHintMatchesRequirement() {
        ...
        WebElement evaluateHint = new WebDriverWait(driver, Duration.ofSeconds(5))
                .until(ExpectedConditions.visibilityOfElementLocated(
        ...
        if (!expectedHint.equals(evaluateHintCopy)) {
            throw new AssertionError("Expected HOTP inline evaluation hint to read \""
                    + expectedHint
                    + "\" but found \""
                    + evaluateHintCopy
                    + "\"");
        ...
        WebElement replayHint = new WebDriverWait(driver, Duration.ofSeconds(5))
                .until(ExpectedConditions.visibilityOfElementLocated(
        ...
        if (!expectedHint.equals(replayHintCopy)) {
            throw new AssertionError("Expected HOTP inline replay hint to read \""
                    + expectedHint
                    + "\" but found \""
                    + replayHintCopy
                    + "\"");
    ...
    void inlineHotpParametersRenderInCompactRow() {
        ...
        WebElement parameterGrid = new WebDriverWait(driver, Duration.ofSeconds(5))
                .until(ExpectedConditions.visibilityOfElementLocated(
        ...
        if (columnDefinitions.length < 3) {
            throw new AssertionError(
                    "Expected inline HOTP parameter grid to expose three columns but found: " + columnTemplate);
        }
        ...
        long compactTrackCount = java.util.Arrays.stream(columnDefinitions)
                .filter(def -> def.contains("5.5rem"))
                .count();
        ...
        if (algorithmClass == null || !algorithmClass.contains("select-compact")) {
            throw new AssertionError("Expected inline HOTP algorithm select to apply compact styling class");
        }
    ...
    void inlineHotpEvaluationSucceeds() {
        ...
        WebElement inlinePanel = new WebDriverWait(driver, Duration.ofSeconds(5))
                .until(ExpectedConditions.visibilityOfElementLocated(
        ...
        WebElement presetContainer = new WebDriverWait(driver, Duration.ofSeconds(5))
                .until(ExpectedConditions.visibilityOfElementLocated(
        ...
        WebElement heading = resultPanel.findElement(By.cssSelector("[data-testid='hotp-result-heading']"));
    ...
    private WebElement locateStatusColumn() {
        ...
        List<WebElement> columnContainers = cardBody.findElements(
                By.xpath("./*[contains(concat(' ', normalize-space(@class), ' '), ' section-columns ')]"));
        if (columnContainers.size() != 1) {
            throw new AssertionError("Expected HOTP evaluate panel to expose a single section-columns container");
        }
        List<WebElement> statusColumns = columnContainers
                .get(0)
                .findElements(
                        By.xpath("./div[contains(concat(' ', normalize-space(@class), ' '), ' status-column ')]"));
    ...
    private void assertResultRowsMatchExpectations(WebElement resultPanel, String expectedOtp) {
        ...
        String statusLabel = statusRow
                .findElement(By.tagName("dt"))
                .getText()
                .trim()
                .replace(":", "")
                .trim();
        ...
        WebElement statusValueNode = statusRow.findElement(By.cssSelector("dd [data-testid='hotp-result-status']"));
