...
class HotpStoredSampleEndpointTest {
    ...
    void storedSampleReturnsMatchingOtp() throws Exception {
        ...
        String responseBody = mockMvc.perform(get("/api/v1/hotp/credentials/{credentialId}/sample", CREDENTIAL_ID)
        ...
        assertEquals(
                generateOtp(HotpHashAlgorithm.SHA1, 6, 7L), response.path("otp").asText());

        assertThat(response.path("metadata").isMissingNode()).isFalse();
        assertThat(response.path("metadata").path("samplePresetKey").asText()).isEqualTo("ui-hotp-demo");
        assertThat(response.path("metadata").path("samplePresetLabel").asText()).isEqualTo("stored-demo");
    ...
    void storedSampleMissingReturns404() throws Exception {
        mockMvc.perform(get("/api/v1/hotp/credentials/{credentialId}/sample", "missing-credential")
                        .accept(MediaType.APPLICATION_JSON))
