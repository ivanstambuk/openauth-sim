...
class Fido2AttestationEndpointTest {
    ...
    private static final String EXPECTED_CLIENT_DATA =
            encode(VECTOR.registration().clientDataJson());
    private static final String EXPECTED_CHALLENGE =
            encode(VECTOR.registration().challenge());
    private static final List<String> EXPECTED_CERTIFICATE_CHAIN_PEM = CERTIFICATE_CHAIN.stream()
            .map(Fido2AttestationEndpointTest::toPemUnchecked)
            .toList();
    private static final String EXPECTED_CREDENTIAL_ID =
            encode(VECTOR.registration().credentialId());
    ...
    void generateSelfSignedAttestationMatchesFixture() throws Exception {
        ...
        assertThat(metadata.get("attestationFormat").asText())
                .isEqualTo(VECTOR.format().label());
