...
final class OpenApiDocumentationTest {
    ...
    void evaluationEndpointsDocumented() throws Exception {
        ...
        assertTrue(paths.has("/api/v1/hotp/evaluate/inline"), "HOTP inline evaluation endpoint missing from OpenAPI");
        ...
        assertNotNull(hotpStoredPost, "OpenAPI document missing POST operation for HOTP stored evaluation");
        JsonNode hotpInlinePost = paths.get("/api/v1/hotp/evaluate/inline").get("post");
        assertNotNull(hotpInlinePost, "OpenAPI document missing POST operation for HOTP inline evaluation");
        assertTrue(
    ...
    void evaluationEndpointsDocumentedYaml() throws Exception {
        String responseBody = mockMvc.perform(get("/v3/api-docs.yaml"))
                .andExpect(status().isOk())
        ...
        assertTrue(
                responseBody.contains("/api/v1/ocra/evaluate"), "OCRA evaluation endpoint missing from OpenAPI YAML");
        assertTrue(
                responseBody.contains("/api/v1/hotp/evaluate"), "HOTP evaluation endpoint missing from OpenAPI YAML");
        assertTrue(
                responseBody.contains("/api/v1/hotp/evaluate/inline"),
                "HOTP inline evaluation endpoint missing from OpenAPI YAML");
        assertTrue(responseBody.contains("post:"), "OpenAPI YAML missing POST operations for evaluation endpoints");
    }
}
