...
class WebAuthnReplayService {
  ...
  private static WebAuthnReplayMetadata buildMetadata(
    ...
    String credentialId =
        result.credentialReference()
            ? (String)
                fields.getOrDefault(
                    "credentialId", Optional.ofNullable(result.credentialId()).orElse(null))
            : null;
    ...
    boolean userVerificationRequired =
        Boolean.parseBoolean(
            String.valueOf(fields.getOrDefault("userVerificationRequired", Boolean.FALSE)));
    String error =
        result
            .error()
  ...
  private static void logTelemetry(TelemetryFrame frame, String credentialSource) {
    ...
    StringBuilder builder =
        new StringBuilder("event=rest.fido2.replay")
            .append(" status=")
            .append(frame.status())
