...
class OcraVerificationService {
    ...
    OcraVerificationResponse verify(OcraVerificationRequest rawRequest, OcraVerificationAuditContext auditContext) {
        ...
        try {
            ...
            switch (result.status()) {
                case MATCH -> {
                    ...
                    TelemetryFrame frame = TelemetryContracts.ocraVerificationAdapter()
                            .status("match", telemetryId, reasonCodeFor(result.reason()), true, null, payload.fields());
                ...
                case INVALID -> handleInvalid(result, envelope, rawRequest, auditContext, telemetryId, durationMillis);
            }
            ...
            TelemetryFrame frame = TelemetryContracts.ocraVerificationAdapter()
                    .validationFailure(telemetryId, ex.reasonCode(), ex.getMessage(), ex.sanitized(), payload.fields());

            logVerification(Level.WARNING, 422, auditContext, frame);

            throw new OcraVerificationValidationException(
                    telemetryId, payload.suite(), ex.field(), ex.reasonCode(), ex.getMessage(), ex.sanitized(), ex);
            ...
            TelemetryFrame frame = TelemetryContracts.ocraVerificationAdapter()
                    .validationFailure(telemetryId, ex.reasonCode(), ex.getMessage(), true, payload.fields());
    ...
    private void handleInvalid(
        ...
        VerificationLogPayload payload = verificationPayloadForResult(result, envelope, outcome, durationMillis);
        ...
        switch (reason) {
            ...
            case CREDENTIAL_NOT_FOUND -> {
                ...
                message = "credentialId %s not found"
        ...
        throw new OcraVerificationValidationException(
                telemetryId, payload.suite(), null, reasonCodeFor(reason), "Verification request invalid", true, null);
    }
    ...
    private static VerificationLogPayload verificationPayloadForResult(
        ...
        Map<String, Object> fields = verificationFields(
                suite,
                envelope.credentialSource(),
    ...
    private static VerificationLogPayload verificationPayloadForFailure(
        ...
        String suite = Optional.ofNullable(envelope)
        ...
        String credentialSource = envelope != null ? envelope.credentialSource() : credentialSource(raw);
        String credentialId = envelope != null && envelope.command() instanceof VerificationCommand.Stored stored
                ? stored.credentialId()
                : raw.credentialId();
        String otpHash = hashOtp(envelope != null ? envelope.otp() : raw.otp());
        VerificationContext context =
                envelope != null ? envelope.normalized().context() : fallbackContext(raw, suite, credentialSource);
        String fingerprint = suite == null ? null : fingerprint(suite, context);
    ...
    private record CommandEnvelope(
        ...
        static CommandEnvelope from(OcraVerificationRequest request) {
            ...
            if (hasCredential) {
                ...
                VerificationCommand command = OcraVerificationRequests.stored(new OcraVerificationRequests.StoredInputs(
                        request.credentialId(),
                        request.otp(),
                        ctx.challenge(),
                        ctx.clientChallenge(),
