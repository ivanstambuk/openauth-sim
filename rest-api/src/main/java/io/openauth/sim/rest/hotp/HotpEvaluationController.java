...
class HotpEvaluationController {
  ...
  @Operation(
      ...
      description =
          "Validates a submitted OTP against a persisted HOTP credential and advances"
  ...
  ResponseEntity<HotpEvaluationResponse> evaluateStored(
      @RequestBody HotpStoredEvaluationRequest request) {
  ...
  @Operation(
      ...
      description =
          "Validates an OTP using inline HOTP parameters without mutating stored credentials.")
  @ApiResponses(
      value = {
        @ApiResponse(
            responseCode = "200",
  ...
  ResponseEntity<HotpEvaluationResponse> evaluateInline(
      @RequestBody HotpInlineEvaluationRequest request) {
    HotpEvaluationResponse response = service.evaluateInline(request);
    return ResponseEntity.ok(response);
  }

  @ExceptionHandler(HotpEvaluationValidationException.class)
  ResponseEntity<HotpEvaluationErrorResponse> handleValidation(
  ...
  ResponseEntity<HotpEvaluationErrorResponse> handleUnexpected(
