...
class TotpReplayService {
    ...
    TotpReplayResponse replay(TotpReplayRequest request) {
        ...
        String resolvedTelemetryId = String.valueOf(frame.fields().getOrDefault("telemetryId", telemetryId));
    ...
    private ReplayCommand buildStoredCommand(TotpReplayRequest request, String telemetryId, String credentialSource) {
        ...
        Instant evaluationInstant = toInstant(request.timestamp(), "timestamp", telemetryId, credentialSource);
        Optional<Instant> timestampOverride = Optional.ofNullable(
                toInstant(request.timestampOverride(), "timestampOverride", telemetryId, credentialSource));
    ...
    private ReplayCommand buildInlineCommand(TotpReplayRequest request, String telemetryId, String credentialSource) {
        ...
        TotpHashAlgorithm algorithm = parseAlgorithm(request.algorithm(), telemetryId, credentialSource);

        int digits = request.digits() != null
                ? requirePositive(
                        request.digits(), "digits_invalid", "Digits must be positive", telemetryId, credentialSource)
                : 6;

        long stepSeconds = request.stepSeconds() != null
                ? requirePositive(
                        request.stepSeconds(),
    ...
    private TotpReplayMetadata buildMetadata(
            ReplayResult result, String credentialSource, String telemetryId, Map<String, Object> telemetryFields) {

        TotpDriftWindow driftWindow = result.driftWindow();
        String algorithm =
                result.algorithm() != null ? result.algorithm().name() : valueAsString(telemetryFields, "algorithm");
        Integer digits = result.digits() != null ? result.digits() : valueAsInteger(telemetryFields, "digits");
        Long stepSeconds = result.stepDuration() != null
                ? result.stepDuration().getSeconds()
                : valueAsLong(telemetryFields, "stepSeconds");
        Integer driftBackward = driftWindow != null
                ? driftWindow.backwardSteps()
                : valueAsInteger(telemetryFields, "driftBackwardSteps");
        Integer driftForward =
                driftWindow != null ? driftWindow.forwardSteps() : valueAsInteger(telemetryFields, "driftForwardSteps");
    ...
    private TotpHashAlgorithm parseAlgorithm(String algorithm, String telemetryId, String credentialSource) {
    ...
    private Instant toInstant(Long epochSeconds, String field, String telemetryId, String credentialSource) {
    ...
    private static void logTelemetry(TelemetryFrame frame, String credentialSource) {
        ...
        StringBuilder builder = new StringBuilder("event=rest.totp.replay")
                .append(" status=")
                .append(frame.status())
    ...
    private int requirePositive(
            Number value, String reasonCode, String message, String telemetryId, String credentialSource) {
