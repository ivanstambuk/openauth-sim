...
@CommandLine.Command(
    ...
    subcommands = {
      HotpCli.ImportCommand.class,
      HotpCli.ListCommand.class,
      HotpCli.EvaluateCommand.class
    })
public final class HotpCli implements Callable<Integer> {

  private static final String EVENT_PREFIX = "cli.hotp.";
  private static final HotpTelemetryAdapter EVALUATION_TELEMETRY =
  ...
  private int failValidation(
      String event, HotpTelemetryAdapter adapter, Map<String, Object> fields, String message) {
  ...
  abstract static class AbstractHotpCommand implements Callable<Integer> {
    ...
    @CommandLine.ParentCommand HotpCli parent;
  ...
  static final class ImportCommand extends AbstractHotpCommand {
    ...
    public Integer call() {
      ...
      try (CredentialStore store = openStore()) {
        ...
        HotpIssuanceApplicationService.TelemetryStatus status = result.telemetry().status();
        PrintWriter writer =
            status == HotpIssuanceApplicationService.TelemetryStatus.SUCCESS ? out() : err();
