...
public final class OcraCredentialPersistenceAdapter
    implements CredentialPersistenceAdapter<OcraCredentialDescriptor> {
  ...
  public VersionedCredentialRecord serialize(OcraCredentialDescriptor descriptor) {
    ...
    descriptor
        .pinHash()
        .map(SecretMaterial::asHex)
        .ifPresent(pinHex -> attributes.put(ATTR_PIN_HASH, pinHex));
    descriptor
        .allowedTimestampDrift()
        .ifPresent(
            drift -> attributes.put(ATTR_ALLOWED_DRIFT_SECONDS, Long.toString(drift.toSeconds())));
