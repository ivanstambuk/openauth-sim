...
public final class WebAuthnFixtures {
  ...
  private static final Map<String, String> SECTION_ID_OVERRIDES =
      Map.of(
          "16.1.6", "packed-es256",
          "16.1.7", "packed-es384",
          "16.1.8", "packed-es512",
    ...
    for (Object entry : vectorEntries) {
      ...
      try {
        ...
        Map<String, Object> registration =
            asObject(vector.get("registration"), section + ".registration");
        Map<String, Object> authentication =
            asObject(vector.get("authentication"), section + ".authentication");

        AttestedCredentialData attested =
            parseAttestationData(registration, section, algorithm.name());

        byte[] expectedChallenge =
            extractBytes(authentication, "challenge", section + ".challenge");
        byte[] clientDataJson =
            extractBytes(authentication, "clientDataJSON", section + ".clientDataJSON");
        ...
        String privateKeyJwk =
            deriveCredentialPrivateKeyJwk(algorithm, registration, attested.publicKeyCose());

        String id = determineFixtureId(section, title);
        fixtures.put(
            id,
            new WebAuthnFixture(
                id, section, algorithm, storedCredential, assertionRequest, privateKeyJwk));
  ...
  private static AttestedCredentialData parseAttestationData(
    ...
    byte[] attestationObject =
        extractBytes(registration, "attestationObject", section + ".attestationObject");
    Map<String, Object> attestationMap;
    try {
      attestationMap =
          toStringKeyedMap(CborDecoder.decode(attestationObject), section + ".attestationObject");
  ...
  private static String deriveEcJwk(
      WebAuthnSignatureAlgorithm algorithm,
      Map<String, Object> registration,
      Map<Integer, Object> cose) {
    byte[] privateKey = extractOptionalBytes(registration, "credential_private_key").orElse(null);
    if (privateKey == null) {
      return null;
    }
    ...
    String curve =
        switch (algorithm) {
          ...
          case ES512 -> "P-521";
          default ->
              throw new IllegalStateException("Unsupported EC algorithm for JWK: " + algorithm);
        };
  ...
  private static String deriveEd25519Jwk(
      Map<String, Object> registration, Map<Integer, Object> cose) {
    byte[] privateKey = extractOptionalBytes(registration, "private_key").orElse(null);
    if (privateKey == null) {
      privateKey = extractOptionalBytes(registration, "credential_private_key").orElse(null);
    }
    if (privateKey == null) {
      return null;
    }
  ...
  private static final class AttestedCredentialData {
    ...
    private AttestedCredentialData(
        byte[] credentialId, byte[] publicKeyCose, long signatureCounter) {
